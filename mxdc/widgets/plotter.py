...
class Plotter(Gtk.Alignment):
    ...
    def add_line(self, xpoints, ypoints, style='-', name='', lw=1, axis="default", alpha=1.0, color=None, redraw=True, markevery=[]):
        ...
        axis = "default" if axis not in self.axis else axis
    ...
    def redraw(self):
        if not self.grid_mode:
            ...
            self.axis['default'].legend(
                lines, labels, loc='upper left', bbox_to_anchor=(0, 1.075), ncol=len(lines),fancybox=False,
                framealpha=0.0, edgecolor='inherit', borderaxespad=0
            )
        self.canvas.draw_idle()
    ...
    def on_mouse_motion(self, event):
        ...
        if event.inaxes and self.data_type and not self.grid_mode:
            x_name = self.data_type['names'][0]
            x, y = event.xdata, event.ydata
            loc = numpy.searchsorted(self.values.data[x_name], [x])[0]
            if self.cursor_line is None:
                self.cursor_line = default.axvline(x, lw=0.5, color='red', antialiased=None)
                for axis, lines in self.plot_scales.items():
                    for name in lines:
                        y_value = self.values.data[name][loc]
                        ax = self.axis[axis]
                        line = self.lines[name]
                        ...
                        self.cursor_points[name] = ax.text(
                            1, y_value, "◀", color=line.get_color(), transform=trans, ha="right", va="center"
                        )
            else:
