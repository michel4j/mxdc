...
class RsyncApp(object):
    ...
    def run(self):
        command = 'rsync -avz -e "ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null" --modify-window=2 --progress --exclude "*:*" %s %s' % (re.escape(self.src), re.escape(self.dest))
