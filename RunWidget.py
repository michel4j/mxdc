...
class RunWidget(gtk.VBox):
	...
	def on_distance_changed(self,widget,event):
		try:
			distance = float(self.entry['distance'].get_text())
		except:
			distance = 150.0			
		if self.number==0 and self.predictor:
			self.predictor.set_all( keV_to_A(self.energy[0]), distance, 0)
			self.predictor.update()
		self.entry['distance'].set_text('%0.2f' % distance)			

	def on_start_angle_changed(self,widget,event):
	...
	def on_end_angle_changed(self,widget,event):
		start_angle = float(self.entry['start_angle'].get_text())	
		start_frame = int(self.entry['start_frame'].get_text())
		delta = float(self.entry['delta'].get_text())
		try:
	...
	def on_delta_changed(self,widget,event):
		try:
			delta = float(self.entry['delta'].get_text())
		except:
			delta = 1.0
	...
	def on_start_frame_changed(self,widget,event):
		start_angle = float(self.entry['start_angle'].get_text())
		end_angle = float(self.entry['end_angle'].get_text())
		delta = float(self.entry['delta'].get_text())
		try:
	...
	def on_end_frame_changed(self,widget,event):
		start_frame = int( self.entry['start_frame'].get_text() )
		delta = float(self.entry['delta'].get_text())
		try:
